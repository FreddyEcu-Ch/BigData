...
from twython import (
    Twython,
)  # Necesario instalarlo la primera vez de forma aislada: pip install Twython
import timeit
...
dbStringConnection = "mongodb+srv://fpcarrio:Pytank2022@freddy.5b0pfkw.mongodb.net/test"
...
df = pd.read_csv("Data/accountsEMBS(2).csv", encoding="latin-1")
...
for s in twitter_accounts[: len(twitter_accounts)]:
    ...
    rate_limit = twitter.get_application_rate_limit_status()["resources"]["statuses"][
    ...
    while page < 17:
        print(
            "--- STARTING PAGE",
            page,
            "...llamadas a la API restantes estimadas: ",
            rate_limit,
        )
        ...
        if (
            len(d) == 0
        ):  # Este registro es diferentes de las menciones y los ficheros DMS
            print("No hubo tweets devueltos........Desplazandose al siguiente ID")
            break
        # if not d['statuses']:
        #    break
        ...
        if rate_limit < 5:
            print(
                "Se estiman menos de 5 llamadas API ... verifique y haga una pausa de 5 minutos si es necesario"
            )
            rate_limit_check = twitter.get_application_rate_limit_status()["resources"][
                "statuses"
            ]["/statuses/user_timeline"]["remaining"]
            print(
                ".......y el límite de ratio de acceso al API actual es: ",
                rate_limit_check,
            )
            if rate_limit_check < 5:
                print("Quedan menos de 5 llamadas API ... pausando por 5 minutos")
                time.sleep(300)  # PAUSA DE 300 SEGUNDOS
                rate_limit = twitter.get_application_rate_limit_status()["resources"][
                    "statuses"
                ]["/statuses/user_timeline"]["remaining"]
                print(
                    ".......Este es el límite de ratio de acceso al API después de una pausa de 5 minutos: ",
                    rate_limit,
                )
...
print(
    "Número de nuevos tweets añadidos en esta ejecución: ",
    tweets.count_documents({}) - starting_count,
)
